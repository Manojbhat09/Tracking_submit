...
parser.add_argument('--data_dir', type=str, default='/home/ubuntu/argoverse-tracking/test', help='specify the datadir for training')
parser.add_argument('--cfg_file', type=str, default='cfgs/argo_config_sampling_trainfull.yaml', help='specify the config for training')
...
def save_argo_format(sample_id, bbox3d, argo_output_dir, scores, lidar_idx_table):
    data = []
    corners3d = kitti_utils.boxes3d_to_corners3d(bbox3d)
    argo_output_file = os.path.join(argo_output_dir, lidar_idx_table['%06d'%sample_id] + '.json')
    with open(argo_output_file,'w') as f:
        for k in range(bbox3d.shape[0]):
            temp = {}
            temp['center'] = {'x':float(bbox3d[k,2]), 'y':-float(bbox3d[k,0]), 'z':-float(bbox3d[k,1])}
            temp['length'] = float(bbox3d[k, 5])
            temp['width']  = float(bbox3d[k, 4])
            temp['height'] = float(bbox3d[k, 3])
            q = pyquaternion.Quaternion(axis=(0.0, 0.0, 1.0), radians = (np.pi/2. - bbox3d[k, 6]))
...
def save_kitti_format(sample_id,  bbox3d, kitti_output_dir, scores, lidar_idx_table):
    corners3d = kitti_utils.boxes3d_to_corners3d(bbox3d)
    print("--------------------")
    kitti_output_file = os.path.join(kitti_output_dir, lidar_idx_table['%06d'%sample_id] + '.txt')
...
def eval_one_epoch_rpn(model, dataloader, epoch_id, result_dir, logger):
    ...
    for data in dataloader:
        ...
        # model inference
        output_file = os.path.join(seg_output_dir, "Input_"+lidar_idx_table['%06d'%b] + '.npy')
        np.save(output_file, input_data)
        # model inference
        ret_dict = model(input_data)
        output_file = os.path.join(seg_output_dir, "Output_"+lidar_idx_table['%06d'%b] + '.npy')
        np.save(output_file, ret_dict)
        b+=1
        ...
        for bs_idx in range(batch_size):
            ...
            if args.save_result or args.save_rpn_feature:
                ...
                output_file = os.path.join(seg_output_dir, lidar_idx_table['%06d'%cur_sample_id] + '.npy')
#                 if not args.test:
...
#                 else:
                ...
                save_argo_format(cur_sample_id, cur_boxes3d, kitti_output_dir, cur_scores_raw,lidar_idx_table)
...
def eval_one_epoch_rcnn(model, dataloader, epoch_id, result_dir, logger):
    ...
    for data in dataloader:
        ...
        if args.save_result:
            ...
            save_argo_format(sample_id,  roi_boxes3d_np, roi_output_dir, roi_scores, lidar_idx_table)
            save_argo_format(sample_id,  pred_boxes3d_np, refine_output_dir, raw_scores.cpu().numpy(), lidar_idx_table)
        ...
        save_argo_format(sample_id, pred_boxes3d_selected, final_output_dir, scores_selected,lidar_idx_table)
    ...
    for data in dataloader:
        ...
        if args.save_result:
            ...
            for k in range(batch_size):
                ...
                save_argo_format(cur_sample_id, roi_boxes3d_np[k], roi_output_dir,
                                  roi_scores_raw_np[k], lidar_idx_table)
                save_argo_format(cur_sample_id, pred_boxes3d_np[k], refine_output_dir,
                                  raw_scores_np[k], lidar_idx_table)
                output_file = os.path.join(rpn_output_dir, lidar_idx_table['%06d'%cur_sample_id] + '.npy')
                np.save(output_file, output_data.astype(np.float32))
        ...
        for k in range(batch_size):
            ...
            final_total += pred_boxes3d_selected.shape[0]
            #image_shape = dataset.get_image_shape(cur_sample_id)
            save_argo_format(cur_sample_id, pred_boxes3d_selected, final_output_dir, scores_selected, lidar_idx_table)
    
            print(f'Pred boxes: {pred_boxes3d_selected.shape}')
    progress_bar.close()

...
import os
import numpy as np
seg_output_dir = os.path.join("/home/jupyter", "peds_prcnn/PointRCNN-Argoverse/train_out")
b = 0
...
#             print(f'Found {gt_boxes3d[0].shape} gtboxes')
...
#         output_file = os.path.join(seg_output_dir, "Input_"+'%06d'%b + '.npy')
#         save_data = {}
#         for k,v in input_data.items():
#             save_data[k]=v.cpu().numpy()
#         np.save(output_file, save_data)
        
        ret_dict = model(input_data)
        
        
#         output_file = os.path.join(seg_output_dir, "Output_"+'%06d'%b + '.npy')
#         save_data = {}
#         for k,v in ret_dict.items():
#             if 'rpn_cls' in k:
#                 rpn_scores_raw = ret_dict['rpn_cls'][:, :, 0]
#                 rpn_scores = torch.sigmoid(rpn_scores_raw)
#                 seg_result = (rpn_scores > cfg.RPN.SCORE_THRESH).long()
#                 save_data[k]=seg_result.cpu().numpy()
#             else:
#                 save_data[k]=v.detach().cpu().numpy()
